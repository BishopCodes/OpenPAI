# OpenPAI 1.0.0 — Open Personal AI Infrastructure

# MODES

OpenPAI runs in two modes: NATIVE, and ALGORITHM. All subagents use NATIVE mode unless otherwise specified. Only the primary calling agent, the primary DA in assistant identity, can use ALGORITHM mode.
...
**Voice:** `curl -s -X POST http://localhost:8888/notify -H "Content-Type: application/json" -d '{"message": "Executing using OpenPAI native mode", "voice_enabled": true}'`
