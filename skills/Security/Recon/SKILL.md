...
`~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/Recon/`
...
- `naabu` - Port scanning
- Note: Requires security MCP profile (`~/.config/openpai/MCPs/swap-mcp security`)
...
Reports saved to:
- **Work directory** (`~/.config/openpai/MEMORY/WORK/{current_work}/`) - For iterative artifacts during investigation
- **MEMORY/RESEARCH/** (`~/.config/openpai/MEMORY/RESEARCH/YYYY-MM/`) - For pentest engagements and formal assessments
...
~/.config/openpai/MCPs/swap-mcp security
...
Assistant: Activating recon skill with passive-recon workflow.
     ...
     Report: ~/.config/openpai/MEMORY/WORK/{current_work}/recon-example-com/
```
...
**Security Skills:**
- `~/.config/openpai/skills/Investigation/` - Entity and people reconnaissance (OSINT)
- `~/.config/openpai/skills/Security/WebAssessment/` - Web application testing
