...
`~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/Research/`
...
**Artifacts persist** at `~/.config/openpai/MEMORY/RESEARCH/{date}_{topic}/` — the vault survives across sessions.
...
**Working files (temporary work artifacts):** `~/.config/openpai/MEMORY/WORK/{current_work}/`
- Read `~/.config/openpai/MEMORY/STATE/current-work.json` to get the `work_dir` value
...
**History (permanent):** `~/.config/openpai/History/research/YYYY-MM/YYYY-MM-DD_[topic]/`
