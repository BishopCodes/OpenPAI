...
```bash
~/.config/openpai/MEMORY/WORK/{current_work}/
```
...
```
~/.config/openpai/MEMORY/WORK/20260111-172408_extract-alpha-analysis/
├── raw-transcript.txt
...
```bash
~/.config/openpai/History/research/YYYY-MM-DD_description/
```
...
```
~/.config/openpai/History/research/2025-10-26_podcast-analysis/
...
1. **Check if plugins captured the output:**
   ...
   ls -lt ~/.config/openpai/History/research/ | head -5

   # Verify your research directory exists
   ls ~/.config/openpai/History/research/YYYY-MM-DD_description/
   ```

2. **If plugins did NOT capture automatically:**
   ```bash
   # Create directory structure manually
   mkdir -p ~/.config/openpai/History/research/YYYY-MM-DD_description/
...
3. **Confirm all files saved:**
   ```bash
   ls -lah ~/.config/openpai/History/research/YYYY-MM-DD_description/
...
WORK_DIR=$(jq -r '.work_dir' ~/.config/openpai/MEMORY/STATE/current-work.json)

# 2. Work in current work item directory
cd ~/.config/openpai/MEMORY/WORK/${WORK_DIR}/
...
mkdir -p ~/.config/openpai/History/research/$(date +%Y-%m-%d)_podcast-analysis/
...
ls -lah ~/.config/openpai/History/research/$(date +%Y-%m-%d)_podcast-analysis/
...
2. **Get current work directory** - Read `~/.config/openpai/MEMORY/STATE/current-work.json` for `work_dir`
3. **Use work item directory** - Work in `~/.config/openpai/MEMORY/WORK/{work_dir}/`
...
6. **Save to history** - Final outputs to `~/.config/openpai/History/research/YYYY-MM-DD_description/`
7. **Verify capture** - Ensure plugins captured or manually save all files
...
4. Save to history (verify plugins captured output) - working artifacts stay with work item
