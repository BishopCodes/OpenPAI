...
`~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/`
...
**Applies To:** `plugins/SecurityValidator.plugin.ts`, ISC verification
**Implementation:**
```typescript
...
Read and analyze:
- ~/.config/openpai/MEMORY/STATE/current-work.json
...
Run: bun ~/.config/openpai/skills/Utilities/PAIUpgrade/Tools/Anthropic.ts
...
**User Customizations** (`~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/`):
...
|---------------|----------------|-------------------|
| "Check out R Amjad's video on OpenCode" | Points to content instead of extracting it | "@ 5:42, R Amjad shows this technique: [quote]" |
...
| "A new plugin feature" | No description of what it IS | "What It Is (16-32 words): PreToolUse plugins can return additionalContext that gets injected into the model's context before execution, enabling reasoning-based decisions rather than binary blocks." |
