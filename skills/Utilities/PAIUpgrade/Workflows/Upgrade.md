...
- ~/.config/openpai/PAI/USER/TELOS/TELOS.md
- ~/.config/openpai/PAI/USER/TELOS/GOALS.md
- ~/.config/openpai/PAI/USER/TELOS/PROJECTS.md
- ~/.config/openpai/PAI/USER/TELOS/CHALLENGES.md
- ~/.config/openpai/PAI/USER/TELOS/STATUS.md
...
- Read ~/.config/openpai/MEMORY/STATE/current-work.json
...
2. Active plugins (list with triggers)
...
2. **GitHub Commits:** For relevant changes:
   - Extract the actual code pattern or plugin signature
...
1. Load channel config:
   bun ~/.config/openpai/PAI/Tools/LoadSkillConfig.ts ../youtube-channels.json
...
4. For NEW videos, extract transcripts:
   bun ~/.config/openpai/PAI/Tools/GetTranscript.ts '<video-url>'
...
DO NOT return vague summaries like 'discusses OpenCode features'.
...
1. Look in ~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/
...
1. Load the github_trending config from user-sources.json:
   Read ~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/user-sources.json
...
| # | Discovery | Source | Why It's Interesting | PAI Relevance |
|---|-----------|--------|---------------------|---------------|
| 1 | PreToolUse plugins can inject reasoning context | claude-code v2.1.16 | Hooks can now return `additionalContext` that Claude reasons about before tool execution — this is a paradigm shift from binary block/allow to intelligent security | SecurityValidator could inject warnings instead of blocking, enabling context-aware security decisions |
...
| # | Recommendation | PAI Relevance | Effort | Files Affected |
|---|---------------|---------------|--------|----------------|
| 1 | Add PreToolUse additionalContext to security plugins | SecurityValidator currently hard-blocks commands — additionalContext enables reasoning-based security that adapts to context | Low | `plugins/SecurityValidator.plugin.ts` |
...
**What It Is (16-32 words):**
PreToolUse plugins can now return an additionalContext field that gets injected into the model's context before tool execution, enabling reasoning-based security rather than hard blocks.

**How It Helps PAI (16-32 words):**
SecurityValidator.plugin.ts currently blocks dangerous commands. With additionalContext, it can inject warnings Claude reasons about, enabling smarter security that adapts to context.
...
**Applies To:** `plugins/SecurityValidator.plugin.ts`
...
**What It Is (16-32 words):**
Native environment variable ${OPENCODE_SESSION_ID} is now available in all plugins and commands, eliminating the need for custom session ID extraction or workaround code.
...
| # | Technique | Source | Priority | PAI Component | Effort |
|---|-----------|--------|----------|---------------|--------|
| 1 | PreToolUse Additional Context | claude-code v2.1.16 | 🔴 | SecurityValidator plugin | Low |
