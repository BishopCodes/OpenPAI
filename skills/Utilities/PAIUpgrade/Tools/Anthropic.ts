...
function generateRecommendation(update: Update): string {
  ...
  if (titleLower.includes('command') || titleLower.includes('slash command')) {
    return `**PAI Impact:** HIGH - Command system update\n` +
      `**Why:** PAI uses slash commands extensively (~/.config/openpai/Commands/). Changes affect our command architecture and user workflows.\n` +
      `**Action:** Review for new command patterns or capabilities. Update PAI's command templates if conventions change.`;
  }

  // Agents/Hooks
  if (titleLower.includes('agent') || titleLower.includes('plugin')) {
    return `**PAI Impact:** HIGH - Agent/Hook system change\n` +
      `**Why:** PAI uses agents (researcher, engineer, architect, etc.) and plugins (load-context, stop-plugin) as core infrastructure components.\n` +
      `**Action:** Check if this affects PAI's agent definitions or plugin configurations. Test existing agent workflows.`;
  }

  // OpenCode releases
  if (type === 'release' && source.includes('claude-code')) {
    return `**PAI Impact:** CRITICAL - Core platform update\n` +
      `**Why:** PAI runs on OpenCode - releases may include new features, breaking changes, or performance improvements.\n` +
  ...
  if (source.includes('cookbook') || source.includes('quickstart') || source.includes('courses')) {
    return `**PAI Impact:** MEDIUM - Implementation patterns\n` +
      ...
      `**Action:** Review for reusable patterns, especially around skills, agents, or OpenCode features. Extract learnings for PAI.`;
  }
  ...
  if (titleLower.includes('doc') || type === 'docs') {
    return `**PAI Impact:** MEDIUM - Capability discovery\n` +
      ...
      `**Action:** Review for new OpenCode features, API capabilities, or configuration options to leverage.`;
  }

  // SDK releases
  if (source.includes('sdk')) {
    return `**PAI Impact:** LOW - SDK update\n` +
      `**Why:** SDK updates are less relevant since PAI uses OpenCode CLI, not raw API SDKs.\n` +
  ...
  if (type === 'blog') {
    return `**PAI Impact:** LOW-MEDIUM - Awareness\n` +
      ...
      `**Action:** Skim for strategic announcements about OpenCode, Skills, or MCP. Track for future planning.`;
  }
...
function assessRelevance(update: Update): 'HIGH' | 'MEDIUM' | 'LOW' {
  ...
  const highKeywords = ['skill', 'mcp', 'command', 'agent', 'plugin', 'breaking', 'opencode'];
...
function generateNarrative(updates: Update[]): string {
  ...
  if (codeUpdates.length > 0) {
    themes.push(`**OpenCode Platform** (${codeUpdates.length} updates)`);
  }
  ...
  // OpenCode analysis (platform)
  if (codeUpdates.length > 0) {
    const highCode = codeUpdates.filter(u => u.priority === 'HIGH').length;
    if (highCode > 0) {
      narrative += `**⚡ PLATFORM UPDATE: OpenCode Changes**\n`;
      narrative += `**${highCode} HIGH-priority** OpenCode updates found. Since PAI runs on OpenCode, platform changes can affect everything. `;
      ...
      narrative += `OpenCode has **${codeUpdates.length} updates** but lower priority. Likely maintenance commits.\n\n`;
    }
  }
  ...
  if (codeUpdates.filter(u => u.priority === 'HIGH').length > 0) {
    topItems.push('3. **OpenCode platform** - Review release notes, test workflows');
  }
