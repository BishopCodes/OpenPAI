...
```bash
~/.config/openpai/skills/[skill-name]/SKILL.md
```
...
```bash
cp -r ~/.config/openpai/skills/[skill-name]/ ~/.config/openpai/History/Backups/[skill-name]-backup-$(date +%Y%m%d)/
```

**Note:** Backups go to `~/.config/openpai/History/Backups/`, NEVER inside skill directories.
...
cd ~/.config/openpai/skills/[SkillName]/Workflows/
...
find ~/.config/openpai/skills/[SkillName]/ -type d -mindepth 2 -maxdepth 3
```
