...
### Skill Directory
```bash
ls ~/.config/openpai/skills/ | grep -i [skillname]
```
...
```bash
ls -la ~/.config/openpai/skills/[SkillName]/
```
...
```bash
bun ~/.config/openpai/skills/[SkillName]/Tools/[ToolName].ts --help
```
...
```bash
grep -l "Intent-to-Flag" ~/.config/openpai/skills/[SkillName]/Workflows/*.md
```
...
**Reference:** `~/.config/openpai/PAI/CliFirstArchitecture.md`
