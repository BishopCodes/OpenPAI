...
const entityIndex = JSON.parse(
  fs.readFileSync('~/.config/openpai/skills/parser/entity-index.json', 'utf-8')
);
...
// Main processing
const entityIndexPath = '~/.config/openpai/skills/parser/entity-index.json';
...
const schema = JSON.parse(
  fs.readFileSync('~/.config/openpai/skills/parser/schema/content-schema.json', 'utf-8')
);
...
processedArticles.forEach((article: any, index: number) => {
  ...
  if (!valid) {
    ...
    const filename = `~/.config/openpai/skills/parser/output/${timestamp}_batch-${index + 1}.json`;
...
```bash
# Check entity counts
cat ~/.config/openpai/skills/parser/entity-index.json | jq '{
...
URLS=("$@")
SKILL_DIR="$HOME/.opencode/skills/parser"
...
const entityIndexPath = `${process.env.HOME}/.opencode/skills/parser/entity-index.json`;
...
processedArticles.forEach((article: any, index: number) => {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
  const filename = `${process.env.HOME}/.opencode/skills/parser/output/${timestamp}_batch-${index + 1}.json`;
