...
`~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/Browser/`
...
**Headless by default.** All automation runs headless. When the user says "show me", open the URL in their preferred browser from `~/.config/openpai/PAI/USER/TECHSTACKPREFERENCES.md`:
...
**Agent definitions:** `~/.config/openpai/agents/BrowserAgent.md` and `~/.config/openpai/agents/UIReviewer.md`
...
**How it works:** OpenCode's `--chrome` flag connects to your actual Chrome browser. Single session, not parallelizable, but has access to all your cookies, sessions, and extensions.
...
**Usage:**
```bash
# Proper way: launch OpenCode with Chrome integration
claude --chrome
```
