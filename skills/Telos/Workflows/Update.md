...
TELOS is {PRINCIPAL.NAME}'s life framework stored in `~/.config/openpai/PAI/USER/TELOS/`. It contains:
...
!`FILE="$1"; CONTENT="$2"; DESCRIPTION="$3"; bun ~/.config/openpai/commands/update-telos.ts "$FILE" "$CONTENT" "$DESCRIPTION"`
...
!`head -50 ~/.config/openpai/PAI/USER/TELOS/updates.md`

## View Specific TELOS File
!`FILE="$1"; cat ~/.config/openpai/PAI/USER/TELOS/"$FILE"`
...
Example:
```bash
bun ~/.config/openpai/commands/update-telos.ts "BOOKS.md" "- *Project Hail Mary* by Andy Weir" "Added favorite book: Project Hail Mary"
```
...
The script is at: `~/.config/openpai/commands/update-telos.ts`

All backups are stored in: `~/.config/openpai/PAI/USER/TELOS/Backups/`

All changes are logged in: `~/.config/openpai/PAI/USER/TELOS/updates.md`
