...
1. **Load Art preferences:**
   ```
   ~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/Art/PREFERENCES.md
   ```
...
3. **Use Theme Constants:**
   ```
   ~/.config/openpai/skills/Remotion/Tools/Theme.ts
   ```

4. **Reference images** (when visual style reference needed):
   ```
   ~/.config/openpai/skills/Media/Art/Examples/
   ```

## PAI Theme Quick Reference

```typescript
import { PAI_THEME } from '~/.config/openpai/skills/Remotion/Tools/Theme'
...
```typescript
import { PAI_THEME, titleScreenStyle, fadeInterpolation } from '~/.config/openpai/skills/Remotion/Tools/Theme'
