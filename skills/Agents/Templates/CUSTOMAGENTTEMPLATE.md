...
- **Built-in agents** (`~/.config/openpai/agents/*.md`): Use this format for the persona section
- **Custom agents** (`~/.config/openpai/custom-agents/*.md`): Use this format for the entire file
...
# === Permissions (OpenCode tool access) ===
permissions:
  allow:
    - "Bash"
    - "Read(*)"
...
|--------|-------------------------|-------------------------------|
| Location | `~/.config/openpai/agents/` | `~/.config/openpai/custom-agents/` |
