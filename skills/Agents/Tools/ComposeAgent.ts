 ...
 *
 * Configuration files:
 *   Base:  ~/.config/openpai/skills/Agents/Data/Traits.yaml
 *   User:  ~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
 *
 * Usage:
...
const CUSTOM_AGENTS_DIR = `${HOME}/.opencode/custom-agents`;
...
/**
 * Save a composed agent to ~/.config/openpai/custom-agents/{slug}.md
 *
 * Produces a CLAUDE CODE COMPATIBLE agent file that can be:
 * 1. Copied to ~/.config/openpai/agents/ and used as a built-in agent
...
function saveAgent(agent: ComposedAgent): string {
  ...
  // Build OpenCode compatible body
...
/**
 * Build a OpenCode compatible agent body (system prompt).
 *
 * Matches the structural format of built-in agents in ~/.config/openpai/agents/*.md:
...
\`\`\`bash
bun run ~/.config/openpai/skills/Agents/Tools/ComposeAgent.ts --load "${slug}"
\`\`\`

Or reconstruct from traits:

\`\`\`bash
bun run ~/.config/openpai/skills/Agents/Tools/ComposeAgent.ts --traits "${agent.traits.join(",")}"
\`\`\`
...
OPTIONS:
  ...
  -s, --save           Save composed agent to ~/.config/openpai/custom-agents/
...
CONFIGURATION:
  Base traits:    ~/.config/openpai/skills/Agents/Data/Traits.yaml
  User traits:    ~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
  Custom agents:  ~/.config/openpai/custom-agents/
