...
| Voice output needed | Named Agent | Pre-mapped to Kokoro voices |
...
**What happens:**
1. Run `bun run ~/.config/openpai/skills/Agents/Tools/ComposeAgent.ts` for EACH agent
2. Use DIFFERENT trait combinations to get unique voices AND colors
3. Each agent gets a personality-matched Kokoro voice and unique color
...
- Trait definitions: `~/.config/openpai/skills/Agents/Data/Traits.yaml`
- Agent template: `~/.config/openpai/skills/Agents/Templates/DynamicAgent.hbs`
- Composition tool: `~/.config/openpai/skills/Agents/Tools/ComposeAgent.ts`

---

## Named Agent Architecture

- **Location**: Individual agent files in `~/.config/openpai/agents/*.md`
