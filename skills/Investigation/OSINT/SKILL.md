...
`~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/OSINT/`
...
```
~/.config/openpai/MEMORY/WORK/$(jq -r '.work_dir' ~/.config/openpai/MEMORY/STATE/current-work.json)/YYYY-MM-DD-HHMMSS_osint-[target]/
```

**Archived reports:**
```
~/.config/openpai/History/research/YYYY-MM/[target]-osint/
```
