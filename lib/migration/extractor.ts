...
export interface ExtractedContent {
  settings: {
    ...
    apiKeys: { kokoro?: string; anthropic?: string } | null;
  };
  userContent: {
    path: string;
    files: string[];
  ...
  plugins: {
    path: string;
    files: string[];
    customPlugins: string[]; // Plugins that differ from default
  };
}
...
export function extractContent(source: InstallationInfo): ExtractedContent {
  const result: ExtractedContent = {
    ...
    plans: { path: '', files: [] },
    plugins: { path: '', files: [], customPlugins: [] },
  };
  ...
  // Extract plugins (only custom ones)
  const pluginsPath = join(source.path, 'plugins');
  if (existsSync(pluginsPath)) {
    result.plugins = {
      path: pluginsPath,
      files: listFilesRelative(pluginsPath),
      customPlugins: [], // Would need hash comparison with default plugins
    };
  }
...
function extractSettings(path: string): ExtractedContent['settings'] {
  ...
  try {
    ...
    result.apiKeys = {
      kokoro: settings.apiKeys?.kokoro || settings.kokoroApiKey,
...
export function copyExtractedContent(
  ...
  const opts = {
    ...
    includePlans: true,
    includePlugins: false, // Default to not copying plugins (use new system plugins)
    ...options,
  };
...
export function formatExtractedContent(content: ExtractedContent): string {
  ...
  if (content.settings.apiKeys?.kokoro) {
    lines.push('  Kokoro TTS: Configured');
  }
