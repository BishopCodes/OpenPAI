  ...
  try {
    ...
    if (content.settings.raw) {
      ...
      if (settingsResult.merged) {
        result.merged.push('opencode.json');
      } else if (settingsResult.error) {
        result.errors.push(`opencode.json: ${settingsResult.error}`);
  ...
  try {
    ...
    switch (strategy) {
      ...
      case 'keep-new':
        ...
        if (extracted.apiKeys?.kokoro || extracted.apiKeys?.anthropic) {
