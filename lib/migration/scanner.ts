...
export const STANDARD_LOCATIONS = [
  join(HOME, '.config/openpai'),
  join(HOME, '.config/openpai-BACKUP'),
  join(HOME, '.config/openpai-old'),
];
...
export function scanInstallation(path: string): InstallationInfo {
  ...
  if (existsSync(pluginsDir)) {
    info.components.plugins = true;
    try {
      info.components.pluginCount = readdirSync(pluginsDir).filter(f => f.endsWith('.ts')).length;
    } catch (e) {
      // Could not count plugins
    }
  }
...
export function formatInstallationInfo(info: InstallationInfo): string {
  ...
  lines.push(`    - opencode.json: ${info.components.settings ? 'Yes' : 'No'}`);
  ...
  lines.push(`    - Hooks: ${info.components.pluginCount}`);
