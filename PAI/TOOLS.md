...
**Location:** `~/.config/openpai/PAI/Tools/Inference.ts`
...
bun ~/.config/openpai/PAI/Tools/Inference.ts --level fast "System prompt" "User prompt"

# Standard (Sonnet) - balanced reasoning, typical analysis
bun ~/.config/openpai/PAI/Tools/Inference.ts --level standard "System prompt" "User prompt"

# Smart (Opus) - deep reasoning, strategic decisions
bun ~/.config/openpai/PAI/Tools/Inference.ts --level smart "System prompt" "User prompt"

# With JSON output
bun ~/.config/openpai/PAI/Tools/Inference.ts --json --level fast "Return JSON" "Input"

# Custom timeout
bun ~/.config/openpai/PAI/Tools/Inference.ts --level standard --timeout 60000 "Prompt" "Input"
```
...
- "deep analysis" → smart
- Plugins use this for sentiment analysis, tab titles, work classification

**Technical Details:**
- Uses Claude CLI with subscription (not API key)
- Disables tools and plugins to prevent recursion
...
bun ~/.config/openpai/PAI/Tools/AddBg.ts /path/to/transparent.png "#EAE9DF" /path/to/output.png

# Add brand background color
bun ~/.config/openpai/PAI/Tools/AddBg.ts /path/to/transparent.png --brand /path/to/output.png
```
...
**Location:** `~/.config/openpai/PAI/Tools/GetTranscript.ts`
...
bun ~/.config/openpai/PAI/Tools/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID"

# Save transcript to file
bun ~/.config/openpai/PAI/Tools/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID" --save /path/to/transcript.txt
```
...
**Voice Configuration:**
- **Voice:** Set via `KOKORO_VOICE` environment variable (default: `af_heart`)
- **Model:** Kokoro 82M (local, no API key needed)
...
**Location:** `~/.config/openpai/PAI/Tools/YouTubeApi.ts`
...
bun ~/.config/openpai/PAI/Tools/YouTubeApi.ts --channel-stats

# Get video statistics
bun ~/.config/openpai/PAI/Tools/YouTubeApi.ts --video-stats VIDEO_ID

# Get latest uploads
bun ~/.config/openpai/PAI/Tools/YouTubeApi.ts --latest-videos
```
...
1. **Add tool file:** Place `.ts` or `.py` file directly in `~/.config/openpai/PAI/Tools/`
...
2. **Document here:** Add section to this file with:
   - Tool location (e.g., `~/.config/openpai/PAI/Tools/ToolName.ts`)
   - Usage examples
