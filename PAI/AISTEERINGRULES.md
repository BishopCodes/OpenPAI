...
Bad: Plugin throws error → remove the entire plugin. Build fails → delete and rewrite the config. Feature broken → rip out the module and replace it.
Correct: Plugin throws error → read the plugin, trace the error, fix the specific line. Build fails → read the error, fix the specific issue. Feature broken → isolate the defect, patch it surgically.
...
**Ask before destructive actions.** Deletes, force pushes, production deploys — always ask first. Use AskUserQuestion with consequences for destructive ops (force push, rm -rf) — don't rely on generic plugin prompts.
