...
| 3 | **OpenCode SDK** | `Bash: claude -p "prompt"` | Isolated execution via subprocess |
...
**System of record: OpenCode task system.** All ISC via `TaskCreate`/`TaskList`/`TaskUpdate`. Task system is sole source of truth — no text-based tracking.
...
- Create PRD at `~/.config/openpai/MEMORY/WORK/{session-slug}/PRD-{YYYYMMDD}-{slug}.md` via `generatePRDTemplate()`.
...
- **Mandatory output format.** Every response MUST use exactly one output format from AGENTS.md Execution Modes (ALGORITHM, NATIVE, ITERATION, or MINIMAL). No freeform output. No exceptions.
...
| **Hook System** | `SYSTEM/THEPLUGINSYSTEM.md` | Event plugins, patterns, implementation |
