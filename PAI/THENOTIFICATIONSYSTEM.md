...
- `voice_id` - Kokoro voice ID (default: {DAIDENTITY.NAME}'s voice)
...
**Voice phase announcements are inline curls in the Algorithm template** (defined in AGENTS.md), not plugins. Each Algorithm phase has a `curl -s -X POST http://localhost:8888/notify` call that gets spoken. The effort level determines which curls fire:
...
**Task completion voice** is handled by `StopOrchestrator.plugin.ts` → `handlers/VoiceNotification.ts`, which extracts the `🗣️` line from the response and POSTs to the voice server.
...
```json
{
  "notifications": {
    ...
    "discord": {
      "enabled": false,
      "webhook": "https://discord.com/api/webplugins/..."
    },
    "thresholds": {
      "longTaskMinutes": 5
    },
...
The notification service is in `~/.config/openpai/plugins/lib/notifications.ts`:
...
In addition to the voice, push, and Discord channels above, PAI plugins emit structured events to `${PAI_DIR}/MEMORY/STATE/events.jsonl`. This is an append-only JSONL file where each line is a typed event (e.g., `algorithm.phase`, `work.created`, `rating.captured`, `voice.sent`). It serves as a unified observability channel that any process can consume by tailing or watching the file.

Events are emitted via `appendEvent()` from `${PAI_DIR}/plugins/lib/event-emitter.ts`, which is synchronous and fire-and-forget. The event type system is defined in `${PAI_DIR}/plugins/lib/event-types.ts` as a TypeScript discriminated union covering 22 event interfaces. This channel is additive -- it does not replace any of the notification channels above, and plugins emit events alongside their existing state writes and notifications.
