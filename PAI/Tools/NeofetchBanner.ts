...
function createNeofetchBanner(): string {
  ...
  const statItems = [
    ...
    { key: "Plugins", value: String(stats.plugins), color: c },
  ...
  const githubUrl = `${f}${RETICLE.topLeft}${RESET} ${tl}github.com/openpai/OpenPAI${RESET} ${f}${RETICLE.topRight}${RESET}`;
...
function createCompactBanner(): string {
  ...
  lines.push(`${s}Skills:${g}${stats.skills}${RESET} ${s}Plugins:${c}${stats.plugins}${RESET}`);
