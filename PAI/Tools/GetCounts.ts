 ...
 * - Plugins: .ts files directly in plugins/ (depth 1)
 ...
 *
 * OUTPUT (JSON):
 *   {"skills":65,"workflows":339,"plugins":18,"signals":3819,"files":172}
 *
 * OUTPUT (--shell):
 *   skills_count=65
 *   workflows_count=339
...
const PAI_DIR = process.env.PAI_DIR || join(HOME, ".config/openpai");
...
function countPlugins(): number {
  let count = 0;
  const pluginsDir = join(PAI_DIR, "plugins");
  try {
...
if (singleKey && singleKey in counts) {
  ...
  console.log(`plugins_count=${counts.plugins}`);
