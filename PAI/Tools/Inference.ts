 ...
 *
 * BILLING: Uses OpenCode CLI with subscription (not API key)
 *
 * ============================================================================
 */
...
export async function inference(options: InferenceOptions): Promise<InferenceResult> {
  ...
  return new Promise((resolve) => {
    ...
    // Also unset OPENCODE so nested `opencode` invocations don't trigger the
    // nested-session guard (plugins run inside OpenCode's environment).
