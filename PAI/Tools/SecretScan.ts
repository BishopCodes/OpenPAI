 ...
 *
 * Usage:
 *   bun ~/.config/openpai/PAI/Tools/SecretScan.ts <directory>
 *   bun ~/.config/openpai/PAI/Tools/SecretScan.ts . --verbose
 *   bun ~/.config/openpai/PAI/Tools/SecretScan.ts . --verify
 *
 * @see ~/.config/openpai/skills/_SYSTEM/Workflows/SecretScanning.md
 */
...
function formatFindings(findings: TruffleHogFinding[], verbose: boolean) {
  ...
  console.log('4. Set up pre-commit plugins to prevent secret commits');
