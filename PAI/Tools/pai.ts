...
const OPENPAI_DIR = join(homedir(), ".config/openpai");
const MCP_DIR = join(OPENPAI_DIR, "MCPs");
const ACTIVE_MCP = join(OPENPAI_DIR, ".mcp.json");
const BANNER_SCRIPT = join(OPENPAI_DIR, "PAI", "Tools", "Banner.ts");
...
async function getLatestVersion(): Promise<string | null> {
  try {
    const response = await fetch(
      "https://storage.googleapis.com/opencode-dist-86c565f3-f756-42ad-8dfa-d59b1c096819/opencode-releases/latest"
    );
...
USAGE:
  ...
  k -l, --local            Stay in current directory (don't cd to ~/.config/openpai)
