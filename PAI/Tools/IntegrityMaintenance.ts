...
const PAI_DIR = process.env.HOME + '/.config/openpai';
...
function generateDescriptiveTitle(changes: FileChange[]): string {
  ...
  // Hook changes
    ...
    const hookNames = paths
      .filter(p => p.includes('plugins/'))
      .map(p => basename(p, '.ts').replace('.plugin', ''));
    if (hookNames.length === 1) {
      title = `${hookNames[0]} Hook Updated`;
    } else if (hookNames.length <= 3) {
      title = `${hookNames.slice(0, 3).join(', ')} Plugins Updated`;
    } else {
...
function determineSignificance(changes: FileChange[]): SignificanceLabel {
  ...
  const hasPlugins = changes.some(c => c.category === 'plugin');
...
function inferChangeType(changes: FileChange[]): ChangeType {
  ...
  if (uniqueCategories.size === 1) {
    ...
    switch (cat) {
      ...
      case 'plugin': return 'plugin_update';
  ...
  if (uniqueCategories.has('plugin')) return 'hook_update';
...
async function generateNarrativeWithAI(
  ...
  const prompt = `You are analyzing a OpenCode session to generate documentation for a PAI (Personal AI Infrastructure) system update.
...
Return ONLY the JSON object, no other text.`;
  ...
  try {
    ...
    const systemPrompt = 'You are analyzing a OpenCode session to generate documentation. Return ONLY valid JSON, no other text.';
  ...
  if (aiNarrative) {
    return {
      narrative: {
        ...
        verification_commands: [`bun ~/.config/openpai/skills/_SYSTEM/Tools/UpdateSearch.ts recent 5`],
        confidence: 'high',
      },
