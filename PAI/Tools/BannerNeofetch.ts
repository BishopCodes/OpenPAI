...
function countSkills(): number {
  const skillsDir = join(OPENPAI_DIR, "skills");
...
function countPlugins(): number {
  const pluginsDir = join(OPENPAI_DIR, "plugins");
  if (!existsSync(pluginsDir)) return 31;
  let count = 0;
  try {
...
function countWorkItems(): number {
  const workDir = join(OPENPAI_DIR, "MEMORY/WORK");
...
function countLearnings(): number {
  const learningDir = join(OPENPAI_DIR, "MEMORY/LEARNING");
...
function countUserFiles(): number {
  const userDir = join(OPENPAI_DIR, "PAI/USER");
...
function createNeofetchBanner(): string {
  ...
  statsLines.push(`${GRADIENT.purple1}⚙${RESET}  ${UI.muted}Plugins${RESET}        ${GRADIENT.purple1}${stats.plugins}${RESET} ${progressBar(stats.plugins, maxPlugins, 10)}`);
  ...
  const githubUrl = `${linkIcon} ${UI.subtext}github.com/${RESET}${GRADIENT.blue1}openpai${RESET}${UI.subtext}/${RESET}${GRADIENT.purple1}OpenPAI${RESET}`;
...
function createCompactBanner(): string {
  ...
  lines.push(`  ${UI.dim}│${RESET}   ${kaiGradient}  ${GRADIENT.cyan1}⚡${RESET}${UI.text}${stats.skills}${RESET} ${GRADIENT.purple1}⚙${RESET}${UI.text}${stats.plugins}${RESET} ${UI.success}💡${RESET}${UI.text}${stats.learnings}${RESET} ${GRADIENT.magenta}🎯${RESET}${UI.text}${stats.model}${RESET}   ${UI.dim}│${RESET}`);
  lines.push(`  ${UI.dim}│${RESET}                                                 ${UI.dim}│${RESET}`);
