...
const OPENPAI_DIR = path.join(process.env.HOME!, ".config/openpai");
const MEMORY_DIR = path.join(OPENPAI_DIR, "MEMORY");
const USERNAME = process.env.USER || require("os").userInfo().username;
const PROJECTS_DIR = path.join(OPENPAI_DIR, "projects", `-Users-${USERNAME}--opencode`);  // OpenCode native storage
...
function categorizeFile(filePath: string): keyof ParsedActivity["categories"] | null {
  ...
  if (PATTERNS.plugins.test(filePath)) return "plugins";
...
function getRelativePath(filePath: string): string {
  const openpaiIndex = filePath.indexOf("/.opencode/");
  if (openpaiIndex === -1) return filePath;
  return filePath.substring(openpaiIndex + 9); // Skip "/.opencode/"
}
...
async function parseEvents(sessionFilter?: string): Promise<ParsedActivity> {
  ...
  if (categories.plugins.length > 0) {
    summaryParts.push(`${categories.plugins.length} hook(s)`);
  }
...
function determineChangeType(activity: ParsedActivity): ChangeType {
  ...
  if (categories.plugins.length > 0) return 'hook_update';
...
function determineSignificance(activity: ParsedActivity): SignificanceLabel {
  ...
  if (categories.plugins.length > 0) return 'moderate';
...
function generateTitle(activity: ParsedActivity): string {
  ...
  // Hook changes - describe what plugins
  if (categories.plugins.length > 0) {
    const hookNames = categories.plugins
...
function generatePurpose(activity: ParsedActivity): string {
  ...
  if (categories.plugins.length > 0) {
    return 'Update plugin system for better lifecycle management';
  }
