 ...
 *
 * Usage:
 *   import { loadSkillConfig } from '~/.config/openpai/PAI/Tools/LoadSkillConfig';
 *   const config = loadSkillConfig<MyConfigType>(__dirname, 'config.json');
 *
 * Or CLI:
 *   bun ~/.config/openpai/PAI/Tools/LoadSkillConfig.ts <skill-dir> <filename>
 */
...
// Constants
const HOME = homedir();
const CUSTOMIZATION_DIR = join(HOME, '.config/openpai', 'PAI', 'USER', 'SKILLCUSTOMIZATIONS');
...
Examples:
  bun LoadSkillConfig.ts ~/.config/openpai/skills/PAIUpgrade sources.json
