 ...
 *
 * MODES:
 *   loop        — Autonomous iteration via `opencode -p` (SDK). Runs until all
 ...
 *
 * EXAMPLES:
 *   algorithm -m loop -p ~/.config/openpai/MEMORY/WORK/auth/PRD-20260207-auth.md
...
const BASE_DIR = process.env.PAI_DIR || join(HOME, ".config/openpai");
...
PRD Resolution:
  Full path     ~/.config/openpai/MEMORY/WORK/auth/PRD-20260207-auth.md
...
async function runLoop(prdPath: string, maxOverride?: number, agentCount: number = 1): Promise<void> {
  ...
  while (true) {
    ...
    if (result.status !== 0) {
      ...
      console.error(`\x1b[31m  opencode -p exited with status ${result.status}\x1b[0m`);
...
function runInteractive(prdPath: string): void {
  ...
  const child = spawn("opencode", [
    ...
    stdio: "inherit",
    cwd: dirname(absPath),
    env: { ...process.env, OPENCODE: undefined },
  });
