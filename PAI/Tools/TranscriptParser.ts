#!/usr/bin/env bun
/**
 * TranscriptParser.ts - OpenCode transcript parsing utilities
 *
 * Shared library for extracting content from OpenCode transcript files.
 * Used by Stop plugins for voice, tab state, and response capture.
 *
 * CLI Usage:
...
/**
 * Safely convert OpenCode content (string or array of blocks) to plain text.
 */
 ...
 * from previous turns when the Stop plugin fires.
 *
...
export function collectCurrentResponseText(transcriptContent: string): string {
  ...
  // OpenCode transcript uses type='user' for both actual user prompts AND
...
export function parseTranscript(transcriptPath: string): ParsedTranscript {
  try {
    ...
    // when the Stop plugin fires. Within the current turn, multiple
