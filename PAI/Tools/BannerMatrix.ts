...
function countLearnings(): number {
  const learningsDir = join(OPENPAI_DIR, "MEMORY", "LEARNING");
...
function createMicroBanner(stats: SystemStats): string {
  ...
  // Stats line
  const statsStr = `${d}  skills:${RESET}${p}${stats.skills}${RESET} ${d}plugins:${RESET}${p}${stats.plugins}${RESET} ${g}[ONLINE]${RESET}`;
  lines.push(statsStr);
...
function createMiniBanner(stats: SystemStats): string {
  ...
  lines.push(`${d} > ${RESET}${p}PLUGINS_ACTIVE${RESET}${d}.: ${RESET}${g}${stats.plugins}${RESET}`);
...
function createNormalBanner(stats: SystemStats): string {
  ...
  const statLabels = [
    ...
    { key: "PLUGINS_ACTIVE", val: String(stats.plugins), color: g, bold: false },
  ...
  const urlLine = `${d}$ git clone ${RESET}${g}https://github.com/openpai/OpenPAI${RESET}`;
