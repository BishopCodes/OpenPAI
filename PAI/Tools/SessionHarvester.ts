#!/usr/bin/env bun
/**
 * SessionHarvester - Extract learnings from OpenCode session transcripts
 *
 * Harvests insights from ~/.config/openpai/projects/ sessions and writes to LEARNING/
 *
 * Commands:
...
// Derive the project slug dynamically from OPENPAI_DIR (works on macOS and Linux)
// macOS: ${HOME}/.config/openpai → -Users-username--opencode
// Linux: /home/username/.config/openpai → -home-username--opencode
const CWD_SLUG = OPENPAI_DIR.replace(/[\/\.]/g, "-");
const PROJECTS_DIR = path.join(OPENPAI_DIR, "projects", CWD_SLUG);
const LEARNING_DIR = path.join(OPENPAI_DIR, "MEMORY", "LEARNING");
