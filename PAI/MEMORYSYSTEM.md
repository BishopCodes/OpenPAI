...
**Location:** `~/.config/openpai/MEMORY/`
...
**Location:** `~/.config/openpai/projects/-Users-{username}--claude/`
...
ls ~/.config/openpai/MEMORY/WORK/ | tail -5
```

### Check ratings
```bash
tail ~/.config/openpai/MEMORY/LEARNING/SIGNALS/ratings.jsonl
```
...
ls -lt ~/.config/openpai/projects/-Users-{username}--claude/*.jsonl | head -5

# View last session events
tail ~/.config/openpai/projects/-Users-{username}--claude/$(ls -t ~/.config/openpai/projects/-Users-{username}--claude/*.jsonl | head -1) | jq .
```

### Check learnings
```bash
ls ~/.config/openpai/MEMORY/LEARNING/SYSTEM/
ls ~/.config/openpai/MEMORY/LEARNING/ALGORITHM/
ls ~/.config/openpai/MEMORY/LEARNING/SYNTHESIS/
```

### Check failures
```bash
# List recent failure captures
ls -lt ~/.config/openpai/MEMORY/LEARNING/FAILURES/$(date +%Y-%m)/ 2>/dev/null | head -10

# View a specific failure
cat ~/.config/openpai/MEMORY/LEARNING/FAILURES/2026-01/*/CONTEXT.md | head -100

# Migrate historical low ratings to FAILURES
bun run ~/.config/openpai/PAI/Tools/FailureCapture.ts --migrate
```

### Check multi-session progress
```bash
ls ~/.config/openpai/MEMORY/STATE/progress/
```

### Run harvesting tools
```bash
# Harvest learnings from recent sessions
bun run ~/.config/openpai/PAI/Tools/SessionHarvester.ts --recent 10

# Generate pattern synthesis
bun run ~/.config/openpai/PAI/Tools/LearningPatternSynthesis.ts --week
```
...
- Moved BACKUPS to `~/.config/openpai/BACKUPS/` (outside MEMORY)
- Renamed RAW-OUTPUTS to RAW
- All directories now ALL CAPS

**2026-01-05:** v1.0 - Unified Memory System migration
- Previous: `~/.config/openpai/history/`, `~/.config/openpai/context/`, `~/.config/openpai/progress/`
- Current: `~/.config/openpai/MEMORY/`
...
- **Hook System:** `THEPLUGINSYSTEM.md`
