...
**Location:** `~/.config/openpai/PAI/Tools/algorithm.ts`
...
bun ~/.config/openpai/PAI/Tools/algorithm.ts -m loop -p <PRD-path> -n 20

# Run in interactive mode (launches a claude session with PRD context)
bun ~/.config/openpai/PAI/Tools/algorithm.ts -m interactive -p <PRD-path>

# Check status of all PRDs
bun ~/.config/openpai/PAI/Tools/algorithm.ts status
```
...
bun ~/.config/openpai/PAI/Tools/algorithm.ts -m loop -p PRD-20260213-auth.md

# Fast loop — 20 max iterations
bun ~/.config/openpai/PAI/Tools/algorithm.ts -m loop -p PRD-20260213-auth.md -n 20

# Parallel loop — 4 agents working on different criteria simultaneously
bun ~/.config/openpai/PAI/Tools/algorithm.ts -m loop -p PRD-20260213-auth.md -n 20 -a 4
```
...
```bash
bun ~/.config/openpai/PAI/Tools/algorithm.ts -m interactive -p PRD-20260213-feature.md
```
...
bun ~/.config/openpai/PAI/Tools/algorithm.ts status -p PRD-20260213-auth

# Pause a running loop (loop checks between iterations)
bun ~/.config/openpai/PAI/Tools/algorithm.ts pause -p PRD-20260213-auth

# Resume a paused loop
bun ~/.config/openpai/PAI/Tools/algorithm.ts resume -p PRD-20260213-auth

# Stop a loop permanently
bun ~/.config/openpai/PAI/Tools/algorithm.ts stop -p PRD-20260213-auth
```
...
|--------|---------|------------|
| Full path | `~/.config/openpai/MEMORY/WORK/20260207-auth/PRD.md` | Used directly |
...
**Location:** `~/.config/openpai/PAI/ACTIONS/pai.ts`
...
**Location:** `~/.config/openpai/PAI/ACTIONS/lib/runner.v2.ts`
...
**Location:** `~/.config/openpai/PAI/ACTIONS/lib/pipeline-runner.ts`
...
```bash
# The Algorithm CLI
alias algorithm="bun ~/.config/openpai/PAI/Tools/algorithm.ts"

# The Arbol CLI
alias pai="bun ~/.config/openpai/PAI/ACTIONS/pai.ts"

# Runners (optional — pai CLI wraps these)
alias arbol-run="bun ~/.config/openpai/PAI/ACTIONS/lib/runner.v2.ts"
alias arbol-pipe="bun ~/.config/openpai/PAI/ACTIONS/lib/pipeline-runner.ts"
```
