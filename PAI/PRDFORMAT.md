...
The AI writes all PRD content directly using Write/Edit tools. Plugins only read PRDs to sync state.
...
```
~/.config/openpai/MEMORY/WORK/{slug}/PRD.md
```
...
PRD is read-only from plugins' perspective:
...
The AI is the sole writer. Plugins only read. work.json is derived state.
...
- **Reference file pattern**: This spec lives at `~/.config/openpai/PAI/PRDFORMAT.md`, not inline in AGENTS.md. Saves ~2,500 tokens/response.
