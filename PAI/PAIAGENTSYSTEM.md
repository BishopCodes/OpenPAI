...
|--------|-----------|-------------|-------------------|
| **Task Tool Subagent Types** | Pre-built agents in OpenCode (Architect, Designer, Engineer, Explore, etc.) | Internal workflow use ONLY | No |
| **Named Agents** | Persistent identities with backstories and Kokoro voices (Serena, Marcus, Rook, etc.) | Recurring work, voice output, relationships | Yes |
...
bun run ~/.config/openpai/skills/Agents/Tools/ComposeAgent.ts --traits "research,enthusiastic,exploratory"
bun run ~/.config/openpai/skills/Agents/Tools/ComposeAgent.ts --traits "research,skeptical,systematic"
bun run ~/.config/openpai/skills/Agents/Tools/ComposeAgent.ts --traits "research,analytical,synthesizing"
```
...
Named agents have rich backstories, personality traits, and mapped Kokoro voices. They provide relationship continuity across sessions.
...
Custom agents are composed on-the-fly from traits using ComposeAgent. Each unique trait combination maps to a different Kokoro voice.
