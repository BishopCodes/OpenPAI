...
{
  "plugins": {
    "SessionStart": ["path/to/plugin.ts"],
    "Stop": ["path/to/plugin.ts"]
  }
}
```
...
```
PRIVATE: ~/.config/openpai/                    PUBLIC: ${PROJECTS_DIR}/PAI/
...
|----------|-------------|----------|
| **Integrity Audits** | 16 parallel agents verify broken references across ~/.config/openpai | `PrivateSystemAudit.md` |
