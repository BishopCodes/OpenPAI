...
**The AI writes ALL PRD content directly using Write/Edit tools.** PRD.md in `MEMORY/WORK/{slug}/` is the single source of truth. The AI is the sole writer — no plugins, no indirection.
...
**What plugins do (read-only from PRD):** A PostToolUse hook (PRDSync.plugin.ts) fires on Write/Edit of PRD.md and syncs frontmatter + criteria to `work.json` for the dashboard. **Plugins never write to PRD.md — they only read it.**
...
- Edit the stub PRD.md (already created at Algorithm entry) to add full content — update frontmatter `effort` field with the determined effort level, and add sections (Context, Criteria, Decisions, Verification) per `~/.config/openpai/PAI/PRDFORMAT.md`
...
4. `~/.config/openpai/MEMORY/STATE/work.json` has the registry of all sessions (populated by read-only PRDSync + PRDStateSync plugins)
...
**Full spec:** `~/.config/openpai/PAI/PRDFORMAT.md` (read during OBSERVE if needed for field details or continuation rules).
