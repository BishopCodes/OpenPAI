...
- Reference `~/.config/openpai/PAI/USER/` for any personalization
...
```bash
ls -1 ~/.config/openpai/skills/ | grep "^_"
```
...
- `~/.config/openpai/PAI/USER/CONTACTS.md` - Contact information
- `~/.config/openpai/PAI/USER/TECHSTACKPREFERENCES.md` - Tech preferences
```
...
`~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/{SkillName}/`
...
```
~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/
...
│   └── VoiceConfig.json         # Kokoro voice mappings
...
1. **Create directory**: `mkdir -p ~/.config/openpai/PAI/USER/SKILLCUSTOMIZATIONS/SkillName`
...
**See:** `~/.config/openpai/skills/Science/Protocol.md` for the full protocol interface
...
**Full documentation:** `~/.config/openpai/PAI/THENOTIFICATIONSYSTEM.md`
...
Read ~/.config/openpai/skills/Media/Art/Aesthetic.md
```
...
```
~/.config/openpai/skills/Createskill/Workflows/CanonicalizeSkill.md
```
...
5. Move backups to `~/.config/openpai/MEMORY/Backups/`
...
**Reference:** `~/.config/openpai/skills/_PERSONAL/_MYSKILL/SKILL.md`
...
**See:** `~/.config/openpai/PAI/CLIFIRSTARCHITECTURE.md` (Workflow-to-Tool Integration section)
...
**See:** `~/.config/openpai/PAI/CLIFIRSTARCHITECTURE.md` (Configuration Flags section) for full documentation
 ...
 *
 * Usage:
 *   bun ~/.config/openpai/skills/SkillName/Tools/ToolName.ts <command> [options]
 *
 * Commands:
